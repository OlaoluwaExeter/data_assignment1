ubuntu@oa413:~/health_project/data$ Read from remote host 10.121.4.235: Operation timed out
 ...
 * Support:        https://ubuntu.com/pro
 ...
 System information as of Sat Oct 25 07:27:51 UTC 2025
...
Last login: Sat Oct 25 05:19:20 2025 from 10.46.73.183
...
mysql> # doctors based at a specific hospital
mysql> SELECT d.doctor_id, d.name AS doctor_name, d.role, h.name AS hospital_name
    -> FROM Doctors d
    -> JOIN Hospitals h ON d.hospital_id = h.hospital_id
    -> WHERE hospital_id= 3;
ERROR 1146 (42S02): Table 'healthdb.Doctors' doesn't exist
mysql> SELECT d.doctor_id, d.name AS doctor_name, d.role, h.name AS hospital_name
    -> FROM doctors d
    -> JOIN Hospitals h ON d.hospital_id = h.hospital_id
    -> WHERE hospital_id= 3;
ERROR 1146 (42S02): Table 'healthdb.Hospitals' doesn't exist
mysql> SELECT d.doctor_id, d.name AS doctor_name, d.role, h.name AS hospital_name FROM doctors d JOIN Hospitals h ON d.hospital_id = h.hospital_id WHERE hospital_id= 3;
ERROR 1146 (42S02): Table 'healthdb.Hospitals' doesn't exist
mysql> SELECT d.doctor_id, d.name AS doctor_name, d.role, h.name AS hospital_name
    -> FROM doctors d
    -> JOIN hospitals h ON d.hospital_id = h.hospital_id
    -> WHERE hospital_id= 3;
ERROR 1054 (42S22): Unknown column 'd.doctor_id' in 'field list'
mysql> SELECT * FROM patients LIMIT 5;
+-----------+------------------+---------------+---------------------------------------------------+---------+-----------+
| person_id | name             | date_of_birth | address                                           | role    | doctor_id |
+-----------+------------------+---------------+---------------------------------------------------+---------+-----------+
...
mysql> SELECT 
    ...
    -> JOIN hospitals h ON d.hospital_id = h.hospital_id
    -> WHERE hospital_id = 5;
ERROR 1052 (23000): Column 'hospital_id' in where clause is ambiguous
mysql> SELECT 
    ->     d.person_id AS doctor_id,
    ->     d.name AS doctor_name,
    ->     d.role,
    ...
    -> FROM doctors d
    -> JOIN hospitals h ON d.hospital_id = h.hospital_id
    -> WHERE h.name = ' Kramer LLC Hospital’;
    '> 
    '> ^C
mysql> SELECT d.doctor_id, d.name AS doctor_name, d.role, h.name AS hospital_name
    -> FROM doctors d
...
mysql> SELECT 
    ...
    -> FROM doctors d
    -> JOIN hospitals h ON d.hospital_id = h.hospital_id
    -> WHERE h.name = ' Acevedo-Lee Hospital';
Empty set (0.00 sec)

mysql> # Patient’s prescription history ordered by date
mysql> SELECT 
    ...
    -> JOIN doctors d ON pr.doctor_id = d.person_id
    -> WHERE p.name = 'James Wilson'
    -> ORDER BY pr.prescription_date DESC;
Empty set (0.00 sec)

mysql> # List all doctors in a specific hospital
mysql> SELECT d.name AS DoctorName, h.name AS HospitalName
    -> FROM doctors d
    -> JOIN hospitals h ON d.hospital_id = h.hospital_id
...
+---------------------+----------------------+
| Dr. Tanner Reynolds | Acevedo-Lee Hospital |
| Dr. Alvin Nelson    | Acevedo-Lee Hospital |
| Dr. Christian Lewis | Acevedo-Lee Hospital |
+---------------------+----------------------+
3 rows in set (0.01 sec)

mysql> 
mysql> SELECT p.prescription_id, pa.name AS PatientName, p.medication, p.prescription_date
    -> FROM prescriptions p
    -> JOIN patients pa ON p.patient_id = pa.person_id
    -> WHERE p.doctor_id = 5
    -> ORDER BY p.prescription_date;
+-----------------+---------------+-------------+-------------------+
| prescription_id | PatientName   | medication  | prescription_date |
+-----------------+---------------+-------------+-------------------+
|             358 | Amanda Barker | Metformin   | 2024-04-14        |
|             336 | Tracy Jensen  | Paracetamol | 2024-04-22        |
+-----------------+---------------+-------------+-------------------+
2 rows in set (0.00 sec)

mysql> # Checking for empty data
mysql> SELECT * FROM doctors WHERE hospital_id IS NULL;
Empty set (0.00 sec)

mysql> # Checking for empty data
mysql> SELECT * FROM patients WHERE doctor_id IS NULL;
Empty set (0.00 sec)

mysql> # Check for missing hospital_id in doctors
mysql> mysql> SELECT * FROM doctors WHERE hospital_id IS NULL
    -> ;
ERROR 1064 (42000): You have an error in your SQL syntax; check the manual that corresponds to your MySQL server version for the right syntax to use near 'mysql> SELECT * FROM doctors WHERE hospital_id IS NULL' at line 1
mysql> # Check for missing hospital_id in doctors
mysql> SELECT * FROM doctors WHERE hospital_id IS NULL;
Empty set (0.00 sec)

mysql> # Prescriptions that were issued by a specific doctor
mysql> SELECT p.prescription_id, pa.name AS PatientName, p.medication, p.prescription_date
    -> FROM prescriptions p
    -> JOIN patients pa ON p.patient_id = pa.person_id
    -> WHERE p.doctor_id = 5
...
mysql> # Prescription history for a specific patient
mysql> SELECT p.prescription_id, p.medication, p.prescription_date, d.name AS DoctorName
    -> FROM prescriptions p
    -> JOIN doctors d ON p.doctor_id = d.person_id
    -> WHERE p.patient_id = 105
    -> ORDER BY p.prescription_date;
+-----------------+-------------+-------------------+-------------------+
| prescription_id | medication  | prescription_date | DoctorName        |
+-----------------+-------------+-------------------+-------------------+
|             157 | Paracetamol | 2023-03-24        | Dr. Blake Johnson |
|             360 | Aspirin     | 2023-04-11        | Dr. Blake Johnson |
+-----------------+-------------+-------------------+-------------------+
2 rows in set (0.00 sec)

mysql> 
mysql> # Register (insert) a new patient
mysql> INSERT INTO patients (name, date_of_birth, address, role, doctor_id)
    -> VALUES (‘120’, 'Sophie Turner', '1995-07-21', '123 Green Street', 'Patient', 3);
ERROR 1136 (21S01): Column count doesn't match value count at row 1
ERROR 1062 (23000): Duplicate entry '999' for key 'patients.PRIMARY'
mysql> INSERT INTO patients (person_id, name, date_of_birth, address, role, doctor_id) VALUES (1099, 'John Doe', '1992-07-10', '23 Main Street', 'patient', 5);
Query OK, 1 row affected (0.08 sec)

mysql> # Verify new patient
mysql> SELECT * FROM patients WHERE person_id = 999;
+-----------+----------+---------------+----------------+---------+-----------+
| person_id | name     | date_of_birth | address        | role    | doctor_id |
+-----------+----------+---------------+----------------+---------+-----------+
|       999 | John Doe | 1992-07-10    | 23 Main Street | patient |         5 |
+-----------+----------+---------------+----------------+---------+-----------+
1 row in set (0.01 sec)

mysql> # Identify which doctor has made the most prescriptions. 
mysql> SELECT 
    ->     d.person_id AS DoctorID,
    ->     d.name AS DoctorName,
    ->     COUNT(p.prescription_id) AS TotalPrescriptions
    -> FROM prescriptions p
    -> JOIN doctors d 
    ...
    -> LIMIT 1;
+----------+--------------------+--------------------+
| DoctorID | DoctorName         | TotalPrescriptions |
+----------+--------------------+--------------------+
...
mysql> 
mysql> # Doctors working at the hospital with the largest capacity.
mysql> 
mysql> SELECT 
    ->     d.person_id AS DoctorID,
    ->     d.name AS DoctorName,
    ...
    ->     h.size AS HospitalBeds
    -> FROM doctors d
    -> JOIN hospitals h 
    ->     ON d.hospital_id = h.hospital_id
    -> WHERE h.size = (SELECT MAX(size) FROM hospitals)
    -> ORDER BY d.name;
+----------+--------------------+------------+-------------------------------------+--------------+
| DoctorID | DoctorName         | DoctorRole | HospitalName                        | HospitalBeds |
+----------+--------------------+------------+-------------------------------------+--------------+
|       88 | Dr. Carly Moyer    | Doctor     | Peters, Anderson and Baker Hospital |          494 |
|       18 | Dr. Kathryn Ray    | Doctor     | Peters, Anderson and Baker Hospital |          494 |
|       73 | Dr. Regina Ryan    | Doctor     | Peters, Anderson and Baker Hospital |          494 |
|       99 | Dr. Scott Reynolds | Doctor     | Peters, Anderson and Baker Hospital |          494 |
+----------+--------------------+------------+-------------------------------------+--------------+
...
mysql> 
mysql> 
mysql> SELECT      d.person_id AS DoctorID,     d.name AS DoctorName,     d.role AS DoctorRole,     h.name AS HospitalName,     h.size AS HospitalBeds FROM doctors d JOIN hospitals h      ON d.hospital_id = h.hospital_id WHERE h.size = (SELECT MAX(size) FROM hospitals) ORDER BY d.name;
+----------+--------------------+------------+-------------------------------------+--------------+
| DoctorID | DoctorName         | DoctorRole | HospitalName                        | HospitalBeds |
+----------+--------------------+------------+-------------------------------------+--------------+
|       88 | Dr. Carly Moyer    | Doctor     | Peters, Anderson and Baker Hospital |          494 |
